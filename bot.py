...
from aiohttp import web  # Web Service на Render
...
# Проверка и создание отсутствующих колонок
...
async def cmd_start(message: types.Message, command: CommandObject, state: FSMContext):
    ...
    if os.path.exists(photo_path):
        ...
        await message.answer_photo(photo=photo, caption=welcome_text, reply_markup=main_keyboard(), parse_mode="Markdown")
    else:
...
async def open_game_menu(message: types.Message, state: FSMContext):
    ...
    if os.path.exists(photo_path):
        photo = FSInputFile(photo_path)
        await message.answer_photo(photo=photo, caption=text, reply_markup=games_menu_keyboard(), parse_mode="Markdown")
    else:
...
async def profile(message: types.Message):
    ...
    if os.path.exists(photo_path):
        photo = FSInputFile(photo_path)
        await message.answer_photo(photo=photo, caption=text, reply_markup=profile_inline_keyboard(), parse_mode="Markdown")
    else:
...
async def referral_program(message: types.Message):
    ...
    text = (
        ...
        f"💸 Награда начисляется за каждого приглашённого пользователя, который пополнит баланс.\n\n"
...
async def approve_deposit(callback: types.CallbackQuery):
    ...
    if user["referrer_id"] and user["ref_rewarded"] == 0 and amount >= MIN_DEPOSIT:
        ...
        await safe_send(ref_id, f"🎉 **Ваш реферал пополнил баланс!**\n Вам начислено **5.00 ₴**.", parse_mode="Markdown")
...
async def admin_find_user_start(callback: types.CallbackQuery, state: FSMContext):
    ...
    await callback.message.answer("🔍 Введите **юзернейм** (с `@` или без) либо **Telegram ID** игрока:", parse_mode="Markdown")
    await callback.answer()
...
async def process_bet(message: types.Message, state: FSMContext):
    ...
    await message.answer("✅ **Выберите сколько раундов играть:**", reply_markup=select_rounds_keyboard(), parse_mode="Markdown")
